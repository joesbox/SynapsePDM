...
#ifndef TFT_STM32_DMA_FILL_ENABLED
  #define TFT_STM32_DMA_FILL_ENABLED 0
#endif

#ifndef TFT_STM32_DMA_FILL_PIXELS
  #define TFT_STM32_DMA_FILL_PIXELS 2048
#endif

#ifndef TFT_STM32_DMA_FILL_MIN_PIXELS
  #define TFT_STM32_DMA_FILL_MIN_PIXELS 1024
#endif
/*
void TFT_eSPI::pushBlock(uint16_t color, uint32_t len)
{
 ...
 //*/
void TFT_eSPI::pushBlock(uint16_t color, uint32_t len){
  #if defined(STM32_DMA) && (TFT_STM32_DMA_FILL_ENABLED != 0)
    if (DMA_Enabled && len >= TFT_STM32_DMA_FILL_MIN_PIXELS) {
      static uint16_t dmaFillBuffer[TFT_STM32_DMA_FILL_PIXELS];
      static uint16_t dmaFillColor = 0;
      static bool dmaFillColorValid = false;
      static bool dmaFillSwapBytes = false;
      uint16_t dmaColor = _swapBytes ? (uint16_t)(color << 8 | color >> 8) : color;

      if (!dmaFillColorValid || dmaFillColor != dmaColor || dmaFillSwapBytes != _swapBytes) {
        for (uint32_t index = 0; index < TFT_STM32_DMA_FILL_PIXELS; index++) {
          dmaFillBuffer[index] = dmaColor;
        }

        dmaFillColor = dmaColor;
        dmaFillColorValid = true;
        dmaFillSwapBytes = _swapBytes;
      }

      while (len != 0) {
        uint32_t chunkPixels = (len > TFT_STM32_DMA_FILL_PIXELS) ? TFT_STM32_DMA_FILL_PIXELS : len;
        uint16_t chunkBytes = (uint16_t)(chunkPixels << 1);

        dmaWait();
        if (HAL_SPI_Transmit_DMA(&spiHal, (uint8_t*)dmaFillBuffer, chunkBytes) != HAL_OK) {
          dmaWait();
          HAL_SPI_Transmit(&spiHal, (uint8_t*)dmaFillBuffer, chunkBytes, HAL_MAX_DELAY);
        }
